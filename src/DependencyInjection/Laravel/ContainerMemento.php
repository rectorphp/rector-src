...
final class ContainerMemento
{
    ...
    /**
     * Removes every registered factory, discovery registration and cached instance whose class is-a
     * $contract, both from the entropy container private storage and from the RectorConfig bookkeeping,
     * so findByContract() cannot resurrect them via reflection autowiring. entropy warms
     * "registeredClasses" during discovery, so a class left only there would come back on the next lookup.
     */
    private static function forgetByContract(RectorConfig $rectorConfig, string $contract): void
    {
        ...
        foreach (['serviceFactories', 'instances', 'registeredClasses', 'boundAbstracts'] as $propertyName) {
