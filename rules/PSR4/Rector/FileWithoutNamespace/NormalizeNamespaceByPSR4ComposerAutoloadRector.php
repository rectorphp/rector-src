...
CODE_SAMPLE
    ...
    private function getClassLikeRenames(Node $node, string $expectedNamespace): array
    {
        ...
        $classLikes = $this->findNonAnonymousClassLikes($node);
        $classLikesNames = array_map(
            function (ClassLike $class): string {
                Assert::notNull($class->name);
                return $class->name->toString();
            },
            $classLikes
        );
        $oldClassLikeNames = array_map(fn (string $name): string => $originalNamespace . '\\' . $name, $classLikesNames);
        $newClassLikeNames = array_map(fn (string $name): string => $expectedNamespace . '\\' . $name, $classLikesNames);
