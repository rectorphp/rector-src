...
final class RegexDashEscapeRector extends AbstractRector implements MinPhpVersionInterface
{
     ...
     */
    private const string THREE_BACKSLASH_FOR_ESCAPE_NEXT_REGEX = '#(?<=[^\\\\])\\\\{2}(?=[^\\\\])#';

    /**
     * @see https://regex101.com/r/YgVJFp/1
     */
    private const string LEFT_HAND_UNESCAPED_DASH_REGEX = '#(\[.*?\\\\(w|s|d))-(?!\])#i';

    /**
     * @see https://regex101.com/r/TBVme9/9
     */
    private const string RIGHT_HAND_UNESCAPED_DASH_REGEX = '#(?<!\[)(?<!\\\\)-(\\\\(w|s|d)[.*]?)\]#i';
