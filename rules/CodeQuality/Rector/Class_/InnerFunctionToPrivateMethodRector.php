...
CODE_SAMPLE
    ...
    {
        $existingMethodNames = array_map(
            fn (ClassMethod $classMethod): string => $classMethod->name->toString(),
            $node->getMethods()
        );
        ...
        foreach ($node->getMethods() as $classMethod) {
            ...
            $innerFunctionNames = array_map(
                fn (Function_ $innerFunction): string => $innerFunction->name->toString(),
                $innerFunctions
            );
    ...
    private function extractInnerFunctions(ClassMethod $classMethod, array &$existingMethodNames): array
    {
        ...
        foreach ($classMethod->stmts ?? [] as $key => $stmt) {
            ...
            if (in_array($functionName, $existingMethodNames, true)) {
                continue;
            }
    ...
    private function createPrivateMethod(Function_ $innerFunction, bool $isStatic): ClassMethod
    {
        ...
        $classMethod = new ClassMethod($innerFunction->name, [
            'flags' => $flags,
            'byRef' => $innerFunction->byRef,
            'params' => $innerFunction->params,
            'returnType' => $innerFunction->returnType,
            'stmts' => $innerFunction->stmts,
            'attrGroups' => $innerFunction->attrGroups,
        ]);
    ...
    private function rewriteCallsToMethod(ClassMethod $classMethod, array $innerFunctionNames, bool $isStatic): void
    {
        $this->traverseNodesWithCallable($classMethod->stmts ?? [], function (Node $node) use (
            ...
            if (! in_array($functionName, $innerFunctionNames, true)) {
                return null;
            }
