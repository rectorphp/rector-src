...
final class ShortenElseIfRector extends AbstractRector implements DeprecatedInterface
{
    ...
    {
        return new RuleDefinition('Shorten `else`/`if` to `elseif`', [
            new CodeSample(
                <<<'CODE_SAMPLE'
class SomeClass
{
...
CODE_SAMPLE
    ...
    public function refactor(Node $node): ?Node
    {
        throw new ShouldNotHappenException(sprintf(
            '"%s" rule is deprecated, as it creates less readable code and breaks the logical reading of the if train',
            self::class
        ));
    }
}
