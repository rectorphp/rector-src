...
CODE_SAMPLE
    ...
    private function processForeachNodeWithReturnInside(
        Foreach_ $foreach,
        Return_ $innerForeachReturn,
        ?Stmt $nextStmt
    ): Return_|null {
        ...
        $coalesce = new Coalesce(new ArrayDimFetch(
            $foreach->expr,
            $checkedNode
        ), $nextStmt instanceof Return_ && $nextStmt->expr instanceof Expr ? $nextStmt->expr : $checkedNode);
