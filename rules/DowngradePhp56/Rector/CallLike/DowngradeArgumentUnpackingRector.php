...
final class DowngradeArgumentUnpackingRector extends AbstractRector
{
    ...
    {
        return new RuleDefinition('Replace argument unpacking by call_user_func_array()', [
            new CodeSample(
                <<<'CODE_SAMPLE'
class SomeClass
{
...
CODE_SAMPLE
    ...
    private function createReflectionInstantiation(New_ $new, array $args): ?Expr
    {
        ...
        if ($new->class instanceof Name) {
            $class = match (strtolower($new->class->toString())) {
                'self' => new FuncCall(new Name('get_class')),
                'static' => new FuncCall(new Name('get_called_class')),
