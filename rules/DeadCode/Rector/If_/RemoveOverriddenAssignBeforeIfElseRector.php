...
final class RemoveOverriddenAssignBeforeIfElseRector extends AbstractRector
{
    ...
    public function getRuleDefinition(): RuleDefinition
    {
        return new RuleDefinition(
            'Remove assign before if/else that overrides the variable in both branches',
            [
                new CodeSample(
                    <<<'CODE_SAMPLE'
class SomeClass
...
CODE_SAMPLE
    ...
    private function isVariableUsedInNode(Node $node, string $variableName): bool
    {
        return (bool) $this->betterNodeFinder->findFirst(
            $node,
            fn (Node $subNode): bool => $subNode instanceof Variable && $this->isName($subNode, $variableName)
        );
    }
}
