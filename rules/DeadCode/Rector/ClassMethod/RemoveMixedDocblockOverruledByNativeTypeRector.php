...
final class RemoveMixedDocblockOverruledByNativeTypeRector extends AbstractRector
{
    ...
    public function getRuleDefinition(): RuleDefinition
    {
        return new RuleDefinition(
            'Remove @param mixed and @return mixed docblock that duplicates an already declared native type',
            [
                new CodeSample(
                    <<<'CODE_SAMPLE'
...
CODE_SAMPLE
    ...
    private function removeMixedParamTags(PhpDocInfo $phpDocInfo, Node $functionLike): bool
    {
        ...
        $phpDocNodeTraverser->traverseWithCallable($phpDocInfo->getPhpDocNode(), '', function (PhpDocNode $phpDocNode) use (
            $declaredParamNames,
            &$hasChanged
        ): ?int {
            ...
            if (! in_array($parameterName, $declaredParamNames, true)) {
                return null;
            }
