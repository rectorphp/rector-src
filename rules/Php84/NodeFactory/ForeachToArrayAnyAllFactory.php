 ...
 */
final readonly class ForeachToArrayAnyAllFactory
{
     ...
     */
    public function refactorToArrayAnyAll(
        Node $node,
        string $functionName,
        bool $negateCondition,
        bool $initialBool,
        bool $rejectElseBranches
    ): ?Node {
        return $this->refactorBooleanAssignmentPattern($node, $functionName, $negateCondition, $initialBool, $rejectElseBranches)
            ?? $this->refactorEarlyReturnPattern($node, $functionName, $negateCondition, $initialBool, $rejectElseBranches);
    }
    ...
    private function refactorBooleanAssignmentPattern(
        ...
        foreach ($node->stmts as $key => $stmt) {
            ...
            $arrowFunction = new ArrowFunction([
                'params' => $params,
                'expr' => $this->applyNegation($condition, $negateCondition),
            ]);
    ...
    private function isValidBooleanAssignmentForeachStructure(
        Foreach_ $foreach,
    ...
    private function isValidEarlyReturnForeachStructure(
        Foreach_ $foreach,
        bool $initialBool,
        bool $rejectElseBranches
    ): bool {
